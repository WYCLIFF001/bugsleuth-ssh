use num_bigint::BigUint;

/// DH group parameters
pub struct DHGroup {
    pub p: BigUint, // Prime modulus
    pub g: BigUint, // Generator
}

impl DHGroup {
    /// Group 14 (2048-bit MODP Group)
    pub fn group14() -> Self {
        let p = BigUint::parse_bytes(
            b"",
            16
        ).unwrap();
        let g = BigUint::from(2u32);
        Self { p, g }
    }

    /// Group 1 (1024-bit MODP Group) - less secure, legacy
    pub fn group1() -> Self {
        let p = BigUint::parse_bytes(
            b"",
            16
        ).unwrap();
        let g = BigUint::from(2u32);
        Self { p, g }
    }
}
